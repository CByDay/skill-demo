...
public class ScheduleServiceImpl extends ServiceImpl<IScheduleMapper, Schedule> implements IScheduleService {
    ...
    private JobKey getJobKey(Schedule schedule) {
        return JobKey.jobKey(schedule.getClassName()
                .concat(".")
                .concat(schedule.getMethodName()));
    }
    ...
    private static CronTrigger getCronTrigger(Schedule schedule) {
        ...
        return TriggerBuilder.newTrigger()
                .withIdentity(TriggerKey.triggerKey(schedule.getClassName()
                        .concat(".")
                        .concat(schedule.getMethodName())))
        ...
        if (enabled) {
            try {
                if (scheduler.checkExists(getJobKey(schedule))) {
                    throw new CustomException("已存在任务调度:".concat(schedule.getClassName())
                            .concat(".")
                            .concat(schedule.getMethodName()));
                }
                ...
                throw new CustomException("任务添加失败:".concat(schedule.getClassName())
                        .concat(".")
                        .concat(schedule.getMethodName()));
            }
    ...
    private void removeSchedule(Schedule schedule) {
        ...
        try {
            ...
            throw new CustomException("移除任务失败:".concat(schedule.getClassName())
                    .concat(".")
                    .concat(schedule.getMethodName()));
        }
    }
            ...
            try {
                if (scheduler.checkExists(jobKey)) {
                    ...
                    Trigger trigger = TriggerBuilder.newTrigger()
                            .startNow()
                            .withIdentity(TriggerKey.triggerKey("TEMP:".concat(schedule.getClassName())
                                    .concat(".")
                                    .concat(schedule.getMethodName())))
                            .build();
                ...
                throw new CustomException("临时执行任务失败:".concat(schedule.getClassName())
                        .concat(".")
                        .concat(schedule.getMethodName()));
            }
    ...
    public void testJob() {
        ...
        System.out.println(tOrderIPage.getSize());
        System.out.println("-----------hello world-------------");
    }
}
